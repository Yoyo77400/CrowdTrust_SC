...
def main():
    ...
    class CrowdTrust(sp.Contract):
        ...
        @sp.entrypoint
        def create_pot(self, title, description, goal, deadline, validation_mode):
...
def test_create_pot():
    ...
    sc.h2("1. Creation reussie avec parametres valides")
    c.create_pot(
        title="Voyage Bali",
    ...
    sc.h2("2. Verification de l'ID auto-incremente")
    c.create_pot(
        title="Cadeau",
...
def test_contribute():
    ...
    c.create_pot(title="Vote Pot", description="d", goal=sp.mutez(1000000000),
        deadline=future, validation_mode=0,
        _sender=creator.address, _now=now, _amount=sp.mutez(0))
    c.create_pot(title="Auto Pot", description="d", goal=sp.mutez(500000000),
        deadline=future, validation_mode=1,
        _sender=creator.address, _now=now, _amount=sp.mutez(0))
...
def test_release():
    ...
    sc.h2("1. Release reussi apres vote majoritaire favorable")
    c.release(0, _sender=creator.address, _now=now, _amount=sp.mutez(0))
    sc.verify(c.data.pots[0].status == 2)

    sc.h2("2. Release reussi en mode auto (sans vote)")
    c.release(2, _sender=creator.address, _now=now, _amount=sp.mutez(0))
    sc.verify(c.data.pots[2].status == 2)

    sc.h2("3. Echec si vote defavorable")
...
def test_refund():
    ...
    sc.h2("2. Refund apres vote negatif (Funded -> Failed)")
...
def test_e2e_negative_vote():
    ...
    c.refund(0, _sender=alice.address, _now=now, _amount=sp.mutez(0))
    sc.verify(c.data.pots[0].status == 3)

    c.claim_refund(0, _sender=alice.address, _now=now, _amount=sp.mutez(0))
    c.claim_refund(0, _sender=bob.address, _now=now, _amount=sp.mutez(0))
    c.claim_refund(0, _sender=charlie.address, _now=now, _amount=sp.mutez(0))
